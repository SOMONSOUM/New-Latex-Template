...
\quad ជុំវិញដែនដីមានដែនម៉ាញេទិចមួយដែរ ហេតុនេះហើយទើបទ្រនិចត្រីវិស័យដែលជា ម្ជុលមេដែក ចង្អុលទៅទិសខាងជើងរហូត។ នៅក្នុងផែនដី មានស្នូលមេដែកធម្មជាតិ $\ce{Fe3O4}$ ដូចនេះលំហជុំវិញផែនដីត្រូវមានដែនម៉ាញេទិច។
\begin{figure}[H]
	\begin{subfigure}{.5\textwidth}
		\centering
		\begin{tikzpicture}
			\node at (0,0) {\includegraphics[scale=.25]{earth}};
			\node[rotate=210] at (-3.85,1) {\includegraphics[scale=.2]{magnetic-need}};
			\draw[<->, >=stealth] (.5,2.55) to (1,2.45);
			\node[rotate=-10] at (.9,2.8) {$11.5^\circ$};
		\end{tikzpicture}
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\begin{tikzpicture}
			\coordinate[label=right:$O$] (O) at (4,4.5);
			\coordinate[label=below:{$\vec{B}$}] (B) at (3,3.1);
			\coordinate[label=below left:{$\vec{B}_{h}$}] (Bh) at (2.9,4.31);
			\coordinate[label=above:{\text{ខ្សែមិនរមូល}}] (D) at (4,6);
			\coordinate[label=below:$G$] (G) at (-.5,4.75);
			\coordinate[label=below right:$M$] (M) at (-.5,3.75);
			\coordinate[label=right:{$\vec{B}_{v}$}] (Bv) at (4,3.15);
			\draw[-] (1,4) to (O) to (4,4.5) to (4,1) to (1,.5) to cycle;
			\draw[-] (4,4.5) to (.5,4.7) to (.5,1) to (4,1);
			\draw[->] (O) to (B);
			\draw[->] (O) to (Bh);
			\draw[->] (O) to (Bv);
			\draw[dashed] (Bh) to (B) to (Bv);
			\draw[->,dashed] (O) to (G) node[above] at (G) {\text{ជើងតាមភូមិសាស្រ្ត}};
			\draw[->,dashed, text=blue] (O) to (M) node[left] at (M) {\text{ជើងម៉ាញេទិច}};
			\node[rotate=11.5] at (O) {\includegraphics[scale=.2]{magnetic-need}};
			\draw[-] (O) to (D);
			\draw pic["$\si{D}$", >=stealth, draw=red, text=magenta, <->, angle eccentricity=1.2, angle radius=2cm] {angle=G--O--M};
			\draw pic["$\si{H}$", >=stealth, draw=red, text=magenta, <->, angle eccentricity=1.5, angle radius=.6cm] {angle=M--O--B};
		\end{tikzpicture}
		\caption{កំប៉ូសង់នៃម៉ាញេទិចផែនដី}
	\end{subfigure}
\end{figure}
	\begin{itemize}
		\item ប្លង់បណ្តោយម៉ាញេទិច $\left(M\right)$ ជាប្លង់ឈរ ដែលក្នុងនោះមានវ៉ិចទ័រអាំងឌុចស្យុង ម៉ាញេទិច $\vec{B}$។
		\item ប្លង់បណ្តោយភូមិសាស្រ្ត $\left(G\right)$ ជាប្លង់ឈរ ដែលកាត់តាមប៉ូលជើង និងប៉ូលត្បូងរបស់ផែនដី។
	\end{itemize}
	\quad ជាទូទៅប្លង់ទាំងពីរត្រង់កន្លែងណាមួយមិនត្រួតស៊ីគ្នាទេ គឹវាខុសគ្នាបន្តិចដែលប្លង់ទាំងពីរបង្កើតបានមុំមួយ $\si{D}$ ហៅថាមុំ ដេគ្លីនេសុង ហើយម្ចុលមេដែកមិនស្ថិតក្នុងប្លង់ដេកទេ ដែលគេអាចបំបែកអាំងឌុចស្យុងម៉ាញេទិចផែនដីជាពីរ គឺអាំងឌុចស្យុងផ្គុំដេក $\vec{B}_{h}$ ឬ $\vec{B}_{o}$ និងអាំងឌុចស្យុងផ្គុំឈរ $\vec{B}_{v}$។
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}
			\coordinate[label=right:{$M$}] (M) at (0,0);
			\coordinate[label=left:{$\vec{B}_{o}$}] (Bh) at (-2,0);
			\coordinate[label=right:{$\vec{B}_{v}$}] (Bv) at (0,-2);
			\coordinate[label=left:{$\vec{B}$}] (B) at (-2,-2);
			\draw[->] (M) to (Bh);
			\draw[->] (M) to (Bv);
			\draw[->] (M) to (B);
			\draw[dashed] (Bh) to (B) to (Bv);
